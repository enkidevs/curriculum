...
Spies (elsewhere referred to as _doubles_ or _mocks_) allow you to replace any function with your own version.
...
The below example checks that the _addTwoAndThree_ calls the _add_ method:
...
Note that _spies_ exists only in the _`describe`_ or _`it`_ block where they are defined.
