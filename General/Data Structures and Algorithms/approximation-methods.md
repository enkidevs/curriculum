...
It is easy to find a function which *grows faster* than another, but that is not the point. In order for *Big-O* to be relevant, it must be as close as possible to *the slowest-growing function, but faster than the one we are looking to bound*.
...
`Big-Theta`, or `Big-Θ`,  describes the **exact bound** of the function. We can express *g(n) is Θ(f(n))* as `g(n) is Ω(f(n))` **and** `g(n) is O(f(n))`[2], for every `n>=n'`:
...
The last function, `Little-o`, satisfies `f(n)<c⋅g(n)`, for every `n>=n'`. In other words, `g(n) is o(f(n))` if `g(n) is O(f(n))` **and not** `Θ(f(n))`.
