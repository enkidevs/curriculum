...
Before a `Thread.sleep` or `Thread.yield` call the Java compiler will not flush writes cached in registers out to shared memory or reload values cached in registers. The `volatile` keyword when used with a variable, guarantees that all threads will always see the most up-to-date value of the variable.
