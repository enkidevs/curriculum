...
First, immutable objects are guaranteed to be **thread safe**. This means that even with concurrent threads accessing the same object, you're guaranteed not to get any concurrency errors, since none of the threads can modify the object after creation. Each thread is guaranteed to be able to access the object and the object will definitely be in the exact same state as the thread 'expects' it to be.
...
conn.request(method, "/")
r1 = conn.getresponse()
conn.request(method, "/", "text=hello")
r2 = conn.getresponse()
```

This code works correctly. However, we've introduced what is known as **'temporal coupling'** which is a dependency between objects in time. In this case, `method = "POST"` must be before either of the connection requests for them to work properly. This can be even more troublesome in languages where the `request` object is fully mutable and can be set and changed at arbitrary times; removing one line which configures the request for some later call breaks the code. Luckily, at least for this specific case, Python does not provide an easy way to mutate a `request`.
