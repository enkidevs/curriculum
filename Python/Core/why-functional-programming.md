...
In this insight, we'll delve a little deeper into the concept of functional programming by analyzing the benefits and drawbacks it can provide. Most of these points are applicable to functional programming as a general programming paradigm and not specifically to functional programming within Python.
...
One sure-fire way of making your programs more easy to reason about and to avoid bugs is to follow the **KISS** approach. (*Keep It Simple, Stupid*) Using only pure functions is a way of ensuring you follow this doctrine.
...
Pure functions are functions which abide by a strict set of three rules:
 ...
 - **They produce no side effects.** (They will not change some other external state, the change can only be in the new returned value.)
...
Why? While this approach will work, issues will eventually arise without great care, such as race conditions or difficulty diagnosing bugs. In order to understand the effect of a function with shared state, you would need to gather the entire history of every shared variable that the function uses or affects, which is very difficult and tedious. Function programming style designs away these difficulties.[1]
...
Why is immutability so useful? Using immutable objects to keep track of your program's state means that there is a fully-traceable, completely accurate state history, which is not really possible using traditional imperative programming.
...
What are the four main concepts of functional programming?
...
* They do not rely on any external, mutable state
