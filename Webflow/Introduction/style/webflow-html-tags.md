...
For instance, you can use the "Paragraph" tag to edit all paragraph elements in your webpage by editing just one of them.
...
The list of elements that can be edited without creating a class are:
```html
Body (all pages)
H1 Heading
H2 Heading
...
In the `Selector`, there are 3 possible colors. `Tags` labeled pink, `classes` labeled blue and `states` labeled green.
...
Selecting the pink one and for instance, editing the typography will make all body elements inherit the same rules as well as any elements within them.

So, setting the color of the text to Red in the body element will make all other elements nested in it have the same color text property.
...
An example where all elements are nested inside the body element and we change the body elements typography.

![example-color-change](https://img.enkipro.com/4577f11a25976058e0f71571b80e97fd.gif)

As you can see all elements that have text inherit the body elements styling properties.
