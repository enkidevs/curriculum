...
To create a project from a template go to `dashboard -> new project` then select a template from the list.

To select a template, hover over the desired template and press select:
...
To publish a project while its open go to the top right corner of the current working project and click the "Publish Project" button.
...
For the purpose of the example, we are going to chose the first option.

After selecting the domain and pressing `Publish to Selected Domain` our website will be ready.
...
Underneath the option we selected, we can see the time it took to create our webpage and we can also decide to unpublish the webpage anytime we want to.
