...
We can differentiate between arrays based on whether their size is declared up-front (_static_) or able to change at run-time (_dynamic_).

A good example is the one of a bookshelf. You start by placing the _first_ book in the left side of the shelf, following with each book to the right of the last one, until there is no more space.

In this case, each book has its own _allocated space_ and you can straight up _see the title and author_ on the side of the book.

A _dynamic_ bookshelf would resize itself as soon as there is no more room for another book, while a _static_ one wouldn't.
