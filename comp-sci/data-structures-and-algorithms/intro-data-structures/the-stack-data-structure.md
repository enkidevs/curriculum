...
The items are inserted and removed according to the _last-in, first-out_ (_LIFO_) principle.
...
In a _stack_, you are able to insert, or **push** items whenever you want, but you could only retrieve, or **pop** the top element.

### Stack ADT

Independent of any language, a stack data structure should provide the following update methods:

- `push(e)` : _Adds_ element _e_ to the top of the stack.
- `pop()` : If the stack is not empty, it _removes_ and _returns_ the top element.

And three other helper methods:

- `top()` : _Returns_ the top element.
- `isEmpty()` : _Returns_ `true` if there are no elements in the stack, `false` otherwise.
