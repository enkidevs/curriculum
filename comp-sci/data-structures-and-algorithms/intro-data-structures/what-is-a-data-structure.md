...
If this is unclear, let's fall back to an example: _clothes in a wardrobe_. Usually, there are two types of people:
...
Both are valid ways of doing laundry. The first one requires more work up-front, but less when it comes to using them. The second one takes less time when it comes to storing them, but makes the _operation_ of finding clothes in the morning more challenging.
...
We might describe the respective _pattern_ of information in these two lists as a "sorted list" and an "unsorted list".
