...
- `TCP` is **connection oriented** – once a connection is established, data can be sent _to_ and _from_ both parties.
- `UDP` is a simpler, connectionless Internet protocol. Multiple messages are sent as packets in chunks using `UDP`.

#### TCP

Transmission Control Protocol is the most widely used protocol over the Internet mostly due to its _reliability_.
...
After both computers are aware that the transfer is over (there are no packets missing), the connection is _dropped_.
