...
Keep in mind that in **OS X** the `-a` flag is implied, while on **Linux** running it with no flags will only show _active_ connections.

To show only _inactive_ connections:
...
An example output for the _loopback_ on **Linux** is:
