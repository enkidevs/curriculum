...
Keeping this in mind, one is able to _revert_, _compare_ and even _switch_ between the versions of a project's files, which greatly systemizes a team's workflow.
...
2.  **Centralised Version Control** (CVCS) - developed to make collaboration within a team easier. Instead of a local database, a single _centralized_ server is used to keep all file versions from which team members can _check out_ them.
...
What _type_ of VCS **only** makes use of a local database to keep changes of files under version control?
