...
To explore your project's history, **Git** provides a logging tool like the `git log` command:
```
$ git log
...
If you want to see a specific number of commits, use the `-n` flag followed by a number:
```
$ git log -n 4
# shows 4 commits
```
...
Additional stats such as files changed, or many lines added can be displayed with the `--stat` flag:
```
$ git log --stat
```
...
To search through the commit messages, use the `--grep` flag with either the search string or regular expression:
```
$ git log --grep="\S+@\S+"
```
