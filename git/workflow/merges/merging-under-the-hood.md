...
Because there is not divergence between the branches, **Git** can simply move current branch pointer to the last commit of the **targeted** branch.

### 2. No fast-forward

In the case of a fast-forward merge, you can use the `--no-ff` flag to change the behaviour of the merge. Instead of simply moving the HEAD pointer forward, a new `commit` is created.
