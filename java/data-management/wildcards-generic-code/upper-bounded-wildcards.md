...
To solve this, we can use the wildcard `?` in place of a specific type. However, we do not want to accept _any_ type; only ones that are subclasses of `Number`. To do this we can add an upper bound to the wildcard like this: `List<? extends Number>`:
