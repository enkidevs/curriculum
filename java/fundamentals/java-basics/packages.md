...
**Packages** are a way of organizing Java classes. The Java packages can be stored in files having the _.jar_ extension, allowing classes to be imported as a group rather than individually.
...
New packages can also be created whenever needed, in case of large projects. Since the package creates a new _namespace_, there won't be any class names conflicts with classes in other packages.
