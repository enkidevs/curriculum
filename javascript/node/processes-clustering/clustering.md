...
In order to avoid processing power bottlenecks the `cluster` module can be used to split a single process into multiple ones (_workers_). Child processes share the same server ports with the master process.
