...
Another feature of npm is allowing the user to set values which would be seen as _environmental variables_ in _scripts_. All re-used values should be declared inside the _config_ object:
...
This would add an entry in the _~/.npmrc_ file (`packageName:js_dev=js/backend/`), which is read at run time and changes the path on the local machine.

It is a good idea to include default values as it ensures the scripts can be run without modification. Take _port_ for example, in a _server.js_ file you could retrieve it by:
