...
## Footnotes
[1:keys]
To get a better understanding of this, let's take a look at how React decides when it should render or re-render a component. In its rawest form, a React component is just an object with a set of properties. Each time a change occurs in these properties, React will compare the new and old versions and, if a difference is detected, it will redraw the component. A unique `key` is needed so that React knows the order of elements. If the order changes, React will then know to to re-render the elements at their new positions.
