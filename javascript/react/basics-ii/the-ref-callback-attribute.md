...
The `ref` is a special _attribute_ in **React** that is attachable to any `component` or **DOM** `element`.

**Refs** should be _callback functions_ that will be automatically called when the targeted component is mounted or unmounted.
...
- the underlying **DOM** element or **React** component _instance_ when `<input />` is **mounted**
...
Using `ref`s implies an _imperative_ approach to a problem, while **React** was designed to be _declarative_. Therefore, to enforce **React**'s data flow it's recommended not to overuse `ref`s.
...
Complete the following `render()` function such that, after _mounting_, the `<input />` element can be accessed via `this.myInput`:
