...
Some common used _special characters_ are:
...
[4:Example]
`* * * * * - % a minute has passed` will output _a minute has passed_ to the console. Each `%` represents a new line.
[5:Example]
...
[6:Sunday]
`Sun` can be both `0` and `7`. _0-6_ means Sunday to Saturday, while _1-7_ means Monday to Sunday.The same is true for months. The notation depends on the distribution.
