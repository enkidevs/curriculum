...
SSH, the acronym for _Secure Socket Shell_, is a protocol that provides a safe way of accessing _remote systems_. It can be seen as a more secure version of Telnet, by ensuring _strong authentication_ and _secure encrypted data transfer_ between two computers over a possible insecure Internet connection.
...
The old version used _public-key cryptography_[1] to authenticate to the remote machine, which SSH-2 replaced with _Deffie-Helman key exchanged_[2]. There are several encryption methods available in SHH-2, but the most used ones are _AES_[3] and _Blowfish_[4].
