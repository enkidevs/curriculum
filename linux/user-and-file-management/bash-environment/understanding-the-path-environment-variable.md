...
**Note**: the fact that `:`'s are used to delimit the directories here is a matter of _convention_. Other parts of the system assume the list is `:`-delimited, but if in the 1970s we had settled on `,` or `;` instead, we would still be using that today.
