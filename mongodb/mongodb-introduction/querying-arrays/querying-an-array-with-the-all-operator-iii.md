...
> 💡 The `$all` operator behaves similarly to the `$and` operator.
...
> 💡 The two methods below give the exact same result - any documents with a `spells` array containing the "Bite" value.
