...
But also _unzip_ them when used together with `*` ( the spread operator ).
...
The `sorted()` method is the equivalent of `sort()` in python3. It sorts the tuples formed from the first `zip` comparing them by the _1st_ element of each tuple.
