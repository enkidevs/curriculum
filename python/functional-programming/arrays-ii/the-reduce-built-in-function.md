...
The `reduce` function is stored in the `functools` module. It allows us to apply a function sequentially to all items in a given iterable, but is different from `map` in that it applies the given function cumulatively.
...
This continues until no elements are left in the iterable and all elements in the iterable have effectively been *reduced* into a single value.
