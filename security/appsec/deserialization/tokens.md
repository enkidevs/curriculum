...
instead use a **session token** to identify a request, and use the session server to store data related to that session token. This will prevent users from directly modifying their state.
