...
Probably the most used data types in tables are _numeric values_. They can be used for `id`s, `stats`, `damage points` etc. Even _true/false_ (`bool`s or `boolean`s) values are considered to be numeric types.
...
- `bit(n)`: where _n_ is the maximum length (PSQL, MYSQL)
...
- `numeric(n, d)`: _n_ is the total number of digits and _d_ is the number of digits after the decimal point (PSQL, MySQL, MSSQL)
- `real`: 32bit/4 bytes, single precision (PSQL, MSSQL)
- `float(p)`: _p_ is the precision in bits (MySQL) or a 64bit/8 bytes IEE-754 number (MSSQL)
