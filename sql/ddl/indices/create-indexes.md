...
Indices[1] play a big part in the _performance_ of the SQL Server database. Indices are created on **columns** in tables and they provide a fast way to look up data based on the _values_. For example when we index one **column** and want to search a row based on some _values_ from that column. Then the SQL Server finds that _value_ in the indices and uses it to locate the whole row.
...
Note that a table with an clustered index on it is called _"clustered table"_ and one with non-clustered only is called _"heap"_.
