...
The term **database server** can mean two different things. First, it can be a computer program that provides database services as specified by *client-server model*[1], or it can be the host (hardware part of a database).

A database server, while used as the *hardware* part of a database, is a computer dedicated to performing database storage and retrieval. This computer runs the database software application (DBMS - database management system). The hardware is the location that the host for the database on the network is pointing to.

When referring to the *software*, we are talking about the program that receives and interprets requests. These requests are called **Queries**: requests to store, retrieve, or change data in the database.
...
In *high-scale applications*, the volume of data transactions can be so big that one single computer wouldn't be able to handle the work. In this case the software application may be running on hundreds of servers, and the database server will need to become a group of servers, with one server doing the coordination, and other servers hosting portions of the database.
...
In a **Cluster** of database servers, how many servers perform the coordination task?
???

* Just one of them, while the others are hosting
* All of them do both, hosting and coordination
