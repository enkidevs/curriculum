...
Every HTML element can accept a `style` attribute. As the styling is specific to the element, _no selector_ is needed. Therefore, the code would be the one _inside_ the curly brackets in a `.css` file:
...
_Note:_ Even though there are multiple ways in which you can style an HTML page, the recommended one is using external files, as it splits the resources into _structure_ and _style_.
