...
However, percentages on properties like `padding-top` and `margin-top` are also calculated based on the _width_ of the container, rather than the height.
...
Percentages on properties such as _padding-top_ and _margin-top_ are calculated based on the
