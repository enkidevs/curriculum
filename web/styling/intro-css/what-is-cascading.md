...
When we talk about "Cascading Style Sheets", what do we mean by _cascade_?
Style blocks _cascade_ from the top of the style sheet to the bottom, like a waterfall.
...
Because the selector that sets their color to `green` comes after the selector that sets their color to `red`, it will _override_ the initial value. Paragraphs will still be `bold` as the second selector doesn't specify another value for the `font-weight` property.
