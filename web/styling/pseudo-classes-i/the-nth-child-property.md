...
However, `nth-child` accepts _expressions_ between the parentheses:
...
The counter for n starts at _0_ and ends when it hits the total number of direct children of the element. For the **3n+2** expression, the values table for 6 children is:
...
As the list has _6_ elements, the expression is relevant only for the 2nd and the 5th. However, it saves a lot of typing.

To select all the children from the fifth one to the end of the list, the expression is **n+5**.

Negative values of _n_ can be used to select children from the bottom up. For example, to select items _1 through 5_ and change their color:
